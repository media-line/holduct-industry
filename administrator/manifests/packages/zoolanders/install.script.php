...
class pkg_zoolandersInstallerScript {
	...
	protected function checkRequirements($parent)
	{
		...
		if( version_compare( (string)$joomla_release, (string)$min_joomla_release, '<' ) ) {
			$this->_error = "This package is not compatible with Joomla! v{$joomla_release}, please update it to {$min_joomla_release} or higher and retry the installation.";
			return false;
		}

		/*
		 * make sure ZOO exist and is enabled
		 */
		if (!JFile::exists(JPATH_ADMINISTRATOR.'/components/com_zoo/config.php')
			|| !JComponentHelper::getComponent('com_zoo', true)->enabled) {
			$this->_error = "This package relies on <a href=\"http://www.yootheme.com/zoo\" target=\"_blank\">ZOO</a>, be sure is installed and enabled.";
			return false;
		}
		...
		if( version_compare((string)$zoo_manifest->version, (string)$min_zoo_release, '<') ) {
			$this->_error = "This package is not compatible with ZOO v{$zoo_manifest->version}, please update it to {$min_zoo_release} or higher and retry the installation.";
			return false;
		}
