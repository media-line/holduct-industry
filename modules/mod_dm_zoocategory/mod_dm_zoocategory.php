...
//register base path
$zoo->path->register($path, 'mod_dm_zoocategory');

// register helpers

$zoo->path->register($path, 'helpers');
$zoo->loader->register('DMCategoryModuleHelper', 'helpers:helper.php');
...
if (($category = $categories[$params->get('category', 0)]) && $category->hasChildren()) {
	include(JModuleHelper::getLayoutPath('mod_dm_zoocategory', $params->get('theme', 'list')));
}