<?php if ($isBlog) { ?>
	<article class="uk-article uk-article-blog" <?php if ($permalink) echo 'data-permalink="'.$permalink.'"'; ?>>

		<?php if ($date_published) : ?>
			<?php
			$date_published = ($date_published) ? JHtml::_('date', $date_published, JText::_('d F y')) : '';

			?>
		<?php endif; ?>

		<a class="uk-latest_news-block uk-cover uk-inline-block uk-position-relative uk-text-contrast uk-width-1-1" href="<?php echo $url; ?>">
			<span class="uk-latest_news-block-image uk-display-inline-block uk-width-1-1 uk-height-1-1 uk-position-relative" style="background-image: url('<?php echo $image; ?>');"></span>

			<span class="uk-latest_news-publish_up uk-position-absolute uk-text-small"><?php echo $date_published; ?></span>

			<span class="uk-latest_news-hits uk-position-absolute">
				<i class="icon-eye uk-text-top"></i>
				<span class="uk-text-top uk-text-small uk-text-middle uk-margin-small-left"><?php echo $hits; ?></span>
			</span>

			<span class="uk-latest_news-title uk-position-absolute"><?php echo $title; ?></span>
